...
# This code is modified from train.py in project 2, because I want to process gray scale mode, therefore I have changed some lines.
# The most codes I changed are in ImageSequence. By Tung-Te Lin, 18th October 2019.
...
def create_model(captcha_length, captcha_num_symbols, input_shape, model_depth=5, module_size=2):
  ...
  for i, module_length in enumerate([module_size] * model_depth):
      for j in range(module_length):
          x = keras.layers.Conv2D(8*2**min(i, 3), kernel_size=3, padding='same', kernel_initializer='he_uniform')(x)
...
class ImageSequence(keras.utils.Sequence):
    def __init__(self, directory_name, batch_size, captcha_length, captcha_symbols, captcha_width, captcha_height):
        ...
        print('self directory:',self.directory_name)
    ...
    def __getitem__(self, idx):
        ...
        X = np.zeros((self.batch_size, self.captcha_height, self.captcha_width,1), dtype=np.float32)    #This line is changed to be gray scale mode.
        ...
        for i in range(self.batch_size):
            ...
            image = cv2.imread(os.path.join(self.directory_name, random_image_file),0)  # Using gray scale mode to read the file.
            image = np.reshape(image,(self.captcha_height,self.captcha_width,1))        # Reshaping the image array, then the format could be processed in the previous method.

            processed_data = np.array(image) / 255.0
...
def main():
    ...
    with tf.device('/device:GPU:0'):          # Using GPU to compute it.
    # with tf.device('/device:CPU:0'):
    # with tf.device('/device:XLA_CPU:0'):
        #model = create_model(args.length, len(captcha_symbols), (args.height, args.width, 3))
        model = create_model(args.length, len(captcha_symbols), (args.height, args.width, 1))     # This line is used to process gray scale mode.
